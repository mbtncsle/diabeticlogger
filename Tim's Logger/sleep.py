...
class Sleep(QMdiSubWindow):
	...
	def numbs(self, number):
		self.wind.setText(self.widgets["sleep_qle"], self.widgets["sleep_qle"].text() + str(number))

	# Function for deleting the last number
	def delete(self):
		string = self.widgets["sleep_qle"].text()
		if len(string) > 0:
			self.wind.setText(self.widgets["sleep_qle"], string[:-1])
