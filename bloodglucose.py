...
class Exercise():
    def __init__(self, file, x, y, gui):
        self.FILE_NAME = file
        self.x = x
        self.y = y
        self.windowWidth = 500
        self.windowHeight = 300
        self.windowTitle = "Blood Glucose Window"
    ...
    def initUI(self, gui):
        ...
        # Create the labels
        self.widgets["step_lbl"] = QLabel(gui)
        self.widgets["error_lbl"] = QLabel(gui)
        self.widgets["date_lbl"] = QLabel(gui)

        # Create the editing box
        self.widgets["step_qle"] = QLineEdit(gui)

        # Create the date box
        self.widgets["date_qde"] = QDateEdit(gui)

        # Create the submit button
        self.widgets["qpb"] = QPushButton(gui)

        # Create the graphing button
        self.widgets["graph"] = QPushButton(gui)

        # Create the numpad buttons
        self.widgets["backspace"] = QPushButton(gui)
        for i in range(0, 10):
            self.widgets["numpad" + str(i)] = QPushButton(gui)

        # Move the labels and set their text
        self.wind.topLeftWindowAlign(self.widgets["step_lbl"])
        self.wind.setText(self.widgets["step_lbl"], "Enter blood glucose reading")
        self.wind.topWindowAlign(self.widgets["error_lbl"])
        self.wind.toRight(self.widgets["step_lbl"], self.widgets["error_lbl"])
        self.wind.topWindowAlign(self.widgets["date_lbl"])
        self.wind.leftXAlign(self.widgets["date_lbl"],
                             self.widgets["error_lbl"].pos().x() + self.widgets["error_lbl"].width() * 2)
        self.wind.setText(self.widgets["date_lbl"], "Enter date")

        # Move the editing box and setup the return signal
        self.wind.rightBelow(self.widgets["step_lbl"], self.widgets["step_qle"])
        self.wind.leftWidgetAlign(self.widgets["step_lbl"], self.widgets["step_qle"])
        self.widgets["step_qle"].returnPressed.connect(self.submit)
        ...
        gui.setGeometry(self.x, self.y, self.windowWidth, self.windowHeight)
        gui.setWindowTitle(self.windowTitle)
    ...
    def graph(self):
        try:
            ...
            plt.ylabel("Blood Glucose Reading")
            plt.xlabel("Date")
            plt.show()
    ...
    def submit(self):
        ...
        temp_steps = self.widgets["step_qle"].text()
